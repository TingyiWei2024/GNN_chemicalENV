...
def compute_atom_energy(batch_data):
    ...
    This is a dummy approach for demonstration.
    """
...
def compute_soft_constraints(batch_data, preds):
    ...
    """
    # --- BDE/BDFe is the only constraint we keep active ---
    bde_mean, bdfe_mean = compute_bde_bdfe(batch_data)
    aggregated_bde = bde_mean + bdfe_mean
    loss_bde = (preds * aggregated_bde).mean()

    # --- COMMENT OUT the other constraints so they don't affect the final loss ---
    """
    ...
    # Return only the bde loss (dummy placeholders for the others)
    loss_energy = torch.tensor(0.0, device=preds.device)
...
def train_one_epoch_with_constraints(
    ...
    base_criterion, lambda_bde=0.001,
    # Comment out or ignore these:
    lambda_energy=0.0,
    lambda_c=0.0
):
    """
    Train for one epoch, combining MSE + the BDE/BDFe constraint.
    ...
    for batch_data in loader:
        ...
        # Only BDE is active; energy and c are zero
...
def validate_with_constraints(
    ...
    """
    Validate for one epoch, combining MSE + the BDE/BDFe constraint.
...
def objective(trial):
    ...
    # Soft constraint lambda (others forced to 0)
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(dataset)):
        ...
        # Build the model with uniform hidden dims (using a fixed hidden_dim value)
        hidden_dim = 128  # Fixed hidden_dim (or set a constant value)
        ...
        # Early stopping: record validation loss for each epoch
...
# Use the best hyperparameters (for final retraining)
...
for fold, (train_idx, val_idx) in enumerate(kf.split(dataset)):
    ...
    hidden_dim = 128  # Fixed hidden_dim value
    model = GINE_Regression(
        node_in_dim=1,
        edge_in_dim=2,
        external_in_dim=4,
    ...
    # Save early stopping curve data for this fold
...
if __name__ == '__main__':
    # Run the Dash app
    # Note: The model training and evaluation has already been performed above.
