...
def compute_bde_bdfe(batch_data):
    """
    Original function to compute aggregated BDE/BDFe, but we'll set it inactive.
    """
...
def compute_atom_energy(batch_data):
    """
    Original function to compute "atom energy," but we'll set it inactive.
    """
...
def compute_soft_constraints(batch_data, preds):
    ...
    Returns the three soft constraint losses:
      ...
      - Energy loss
      - C neighbors loss
    Each is defined as mean(pred * aggregator).
    """
    # We'll comment out BDE/BDFe and Energy constraints.
    """
    ...
    """
    # The only active constraint: "C neighbors"
...
def train_one_epoch_with_constraints(
    ...
    base_criterion, lambda_bde=0.0, lambda_energy=0.0, lambda_c=0.001
):
    """
    Train for one epoch, combining MSE + the "C neighbors" constraint.
    Other constraints are disabled (set to 0).
    """
    model.train()
    total_loss = 0.0
    count = 0
...
def validate_with_constraints(
    ...
    lambda_bde=0.0, lambda_energy=0.0, lambda_c=0.001
):
    """
    Validate for one epoch, combining MSE + the "C neighbors" constraint.
    """
    model.eval()
    total_loss = 0.0
    count = 0
...
def objective(trial):
    ...
    # Only "C neighbors" is relevant, so set others to 0.0
    lambda_bde = 0.0
    lambda_energy = 0.0

    # We'll tune only lambda_c
...
# Force bde and energy to zero in final training
lambda_bde = 0.0
lambda_energy = 0.0
